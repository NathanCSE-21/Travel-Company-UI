...
public class TravProfDB implements Serializable{
    ...
    public void insertNewProfile(TravProf newprofile){
        this.travelerList.add("Profile: [agentID: " + newprofile.ID +
                ...
                ", MdContact: " + newprofile.medCond.mdContact +
                ", MdPhone: "+ newprofile.medCond.mdPhone +
                ", allergyType: " + newprofile.medCond.algType +
               ", illnessType: " + newprofile.medCond.illType +"]");


    }

    static String deleteProfile(String agentID, String LastName){
            String profile = findProfile(agentID, LastName);

            if(profile.equals("Profile not found!")){
                return profile;
            }
            else{
                travelerList.remove(profile);
    ...
    static String findProfile(String agentID , String LastName){
        ...
        if(index>=0){
            //System.out.println("The index is: " + currentTravelerIndex);
            return profile;
        }

        while(currentTravelerIndex+1 < numTravelers){
